...
export default class Blockchain {
    ...
    if(transaction.txInputs && transaction.txInputs.length){
      ...
      for(let i = 0; i < transaction.txInputs.length; i++) {
        ...
        if(utxo.findIndex(txo => txo.tx === txi.previousTx && txo.amount >= txi.amount) === 1) {
          return new Validation(false, "Invalid transaction: The TXO is already spent or does not exist");
        }
      }
    }

    //TODO validar as taxas
  ...
  addBlock(block: Block): Validation {
    ...
    if(!validation.success) return new Validation(false, 'Invalid block');
